...
export default function PackageUpdate() {
  ...
  const { data: details } = apiClient.useAxiosSWR(
    id ? `${apiEndpoint.packages.default}/${id}` : null,
    { params: { feature: true, company: true } }
  );
  ...
  useEffect(() => {
    setInitialValues((prev) => ({
      ...
      module: details?.data?.module ?? "",
      durationType: details?.data?.durationType ?? "",
      fee: details?.data?.fee ?? null,
      currency: details?.data?.currency ?? "",
      company: null,
      totalLoadPost: details?.data?.totalLoadPost ?? null,
      totalTruckPost: details?.data?.totalTruckPost ?? null,
      totalQuota: details?.data?.totalQuota ?? null,
      description: details?.data?.description ?? "",
      content:
        details?.data?.content?.length > 0
          ? details?.data?.content
          : [{ value: false, title: "" }],
      features:
        details?.data?.features?.length > 0
          ? details?.data?.features?.map((item) => item?.id)
          : [],
    }));

    if (details?.data?.module) {
      fetchFeaturesByType(details?.data?.module);
    }
  }, [details]);
        ...
        {({
          ...
          <Form>
            ...
            <h2 className="text-[#1c252e] text-2xl font-semibold font-['Public Sans'] leading-loose mt-6">
              Features
              {/* {JSON.stringify(details?.data?.features?.map((item) => item?.id))} */}
            </h2>
