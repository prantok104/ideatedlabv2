...
const VerifyOTP = ({ userEmail }) => {
  ...
  const initialValues = {
    ...
    value: searchParams.get("email") ?? userEmail,
  };
  ...
  const calculateRemainingTime = () => {
    ...
    const timeDiff = storedTime
      ...
      : 0;
    const remainingTime = 300 - timeDiff; // 5 minutes
  ...
  const handleSubmit = async (values) => {
    try {
      const response = await apiClient.post(apiEndpoint.auth.verifyOtp, values);
  ...
  const handleResendOtp = async () => {
    try {
      ...
      // Store current timestamp in local storage and restart the timer
      localStorage.setItem("otpResendTimestamp", new Date().getTime());

      const remainingTime = calculateRemainingTime();
      setTimer(remainingTime);
      setCanResend(false);
      startCountdown(remainingTime); // Restart the countdown after resend
    } catch (error) {
      notify(error?.message, NOTIFY_MESSAGE_ERROR);
    }
  };
