...
import { DASHBOARD_ROUTE, SET_PASSWORD, USER_SELECTED_PACKAGE } from "@/utils/router";
import {
  ACCESS_TOKEN_KEY,
  REGISTERED_EMAIL,
...
const ForgetPasswordOtp = ({ userEmail }) => {
  const formik = useRef();
  ...
  const handleSubmit = async (values) => {
    try {
      const { data } = await apiClient.post(apiEndpoint.auth.resetPasswordOtp, values);
      if (data?.accessToken) {
