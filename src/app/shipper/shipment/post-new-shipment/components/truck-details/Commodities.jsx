...
const Commodities = ({ label, zIndex, name }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState(null);
  const [searchTerm, setSearchTerm] = useState("");
  const [field, meta, helpers] = useField(name);

  const { data: commodityData, error } = apiClient.useAxiosSWR(
  ...
  const commodities = commodityData?.data || [];
  ...
  return (
    <div className="relative w-full mt-6 pt-2">
      ...
      >
        ...
        >
          {field.value ? field.value.name : "Please select commodities"}
        </span>
        ...
        <ul className="max-h-60 overflow-auto">
          {filteredCommodities.map((commodities) => (
            <li
              ...
              onClick={() => handleSelect(commodities)}
            >
              {commodities.name}
              {" ("}
              {commodities.type}
